...
public class ColorBlocks {
    ...
    public static final RegistryEntry<ColorGrassBlock> COLOR_GRASS_BLOCK = REGISTRATE.object("color_grass_block").block(ColorGrassBlock::new).initialProperties(Material.ORGANIC, MaterialColor.GRASS)
            .properties(prop -> prop.tickRandomly().hardnessAndResistance(0.6F).sound(SoundType.PLANT)).simpleItem().register();

    public static final RegistryEntry<Block> COLOR_DIRT = REGISTRATE.object("color_dirt").block(Block::new).initialProperties(Material.EARTH, MaterialColor.DIRT)
            .properties(prop -> prop.hardnessAndResistance(0.5F).sound(SoundType.GROUND)).simpleItem().register();
